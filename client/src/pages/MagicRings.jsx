...
float fade(float t) {
  return t < uFadeIn ? smoothstep(0.0, uFadeIn, t) : 1.0 - smoothstep(uFadeOut, 3.45 - 0.2, t);
}

float ring(vec2 p, float ri, float cut, float t0, float px) {
  float t = mod(uTime + t0, 3.45);
  float r = ri + t / 3.45 * uScaleRate;
  float d = abs(length(p) - r);
  float a = atan(abs(p.y), abs(p.x)) / 1.5707963;
...
export default function MagicRings({
  ...
  return <div ref={mountRef} style={{ width: '100%', height: '100%', filter: blur > 0 ? `blur(${blur}px)` : 'none' }} />;
}